...
# Tu archivo maestro final acumulado de carrera de los jugadores
...
def crear_fact_historico_jugadores():
    ...
    if not os.path.exists(ruta_origen_historico):
        print(f"❌ Error: No se encuentra el archivo maestro histórico en la ruta especificada.")
        return
    ...
    try:
        ...
        print("📊 Generando tabla FACT_JUGADORES_HISTORICO (Hechos acumulados)...")
        
        # Lista estricta de columnas deseadas (SIN la columna 'Nombre')
        cols_fact_historico = [
        ...
        # Ordenamos de mayor a menor por puntos totales para tener una vista atractiva de los líderes históricos
        ...
        print("\n✅ ¡Hechos históricos generados con éxito!")
        print(f"🏀 Total de leyendas históricas en la FACT: {len(df_fact_hist)}")
        print(f"📊 Columnas totales guardadas: {len(df_fact_hist.columns)} (ID Jugador + ID_Equipo + 35 métricas de carrera)")

    except Exception as e:
        print(f"❌ Ocurrió un error durante el procesamiento del histórico: {e}")
