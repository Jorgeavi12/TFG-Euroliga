...
def unificar_all_seasons_jugadores():
    ...
    if not os.path.exists(ruta_dim_equipos):
        print("❌ Error Crítico: No se encontró 'Dim_Equipos.csv'. Lo necesitamos para insertar los IDs de equipo.")
        return

    # Cargamos el mapa de Nombre Largo -> ID Corto
    ...
    for archivo in archivos:
        ...
        print(f"⚙️ Leyendo temporada {anio}...")
    ...
    # Guardamos el archivo maestro final manteniendo tus separadores intactos
    ...
    print(f"📊 Total de registros combinados: {len(df_maestro_jugadores)}")
