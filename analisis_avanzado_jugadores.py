...
def calcular_enriquecimiento_final(df):
    ...
    # 2. Limpieza y conversión (Internamente siempre trabajamos con puntos)
    ...
    df['eFG%'] = (((tca + 0.5 * df['T3a']) / tci.replace(0, 1)) * 100).round(2) # En % para evitar 0.xxxx
    ...
    # --- NUEVO CÁLCULO CORREGIDO: OREB% Y DREB% PARTIDO A PARTIDO ---
    # 1. Agrupamos por partido y por cada equipo para saber cuánto reboteó cada uno internamente
    ...
    # 5. Aplicamos tus fórmulas exactas de baloncesto avanzado partido a partido
    # OREB% = Reb_Off del jugador / (Reb_Off Totales de su Equipo + Reb_Def Totales del Rival)
    df['OREB%'] = ((df['Reb_Off'] / (df['Equipo_Reb_Off'] + df['Rival_Reb_Def']).replace(0, 1)) * 100).round(2)

    # DREB% = Reb_Def del jugador / (Reb_Def Totales de su Equipo + Rival_Reb_Off Totales del Rival)
    df['DREB%'] = ((df['Reb_Def'] / (df['Equipo_Reb_Def'] + df['Rival_Reb_Off']).replace(0, 1)) * 100).round(2)
...
for archivo in archivos:
    try:
        ...
        # EL TRUCO FINAL: Guardamos usando la coma como separador decimal para Excel
        df.to_csv(archivo, index=False, sep=';', decimal=',', encoding='utf-8-sig')
        print(f"✅ Optimizado para Excel: {os.path.basename(archivo)}")
    except Exception as e:
        print(f"❌ Error en {os.path.basename(archivo)}: {e}")

print("\n¡Listo! Abre los archivos ahora. Si ves #####, solo ensancha la columna.")