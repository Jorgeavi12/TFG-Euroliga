...
def procesar_bloque_equipos(archivo):
    ...
    cols_stats = [
        ...
        'Robos Local', 'Robos Visitante', 'Tapones Local', 'Tapones Visitante',  # 👈 Aseguramos numéricas
    ...
    for eq in equipos:
        ...
        L = df[df['Equipo Local'] == eq]
        V = df[df['Equipo Visitante'] == eq]
        ...
        rob = L['Robos Local'].sum() + V['Robos Visitante'].sum()      # 👈 Acumulado robos
        tap = L['Tapones Local'].sum() + V['Tapones Visitante'].sum()  # 👈 Acumulado tapones
        ...
        stats = {
            ...
            'Equipo': eq,
            'ID Equipo': id_equipo,
            'Entrenador': entrenador,  # 👈 Añadido al lado de ID Equipo
            'Partidos': partidos,
            'W': wins,
            'L': partidos - wins,
            ...
            'Robos_PG': round(rob / partidos, 2),      # 👈 Nuevo promedio robos
            'Tapones_PG': round(tap / partidos, 2),    # 👈 Nuevo promedio tapones
            ...
            # --- ANTES DE DEAN OLIVER ---
            'Pts_Permitidos_PG': round(pts_rival / partidos, 2), # 👈 Columna de puntos permitidos
            ...
            # Auxiliares para el USG% de jugadores
            'Equipo_FGA': fga,
            'Equipo_FTA': t1i,
            'Equipo_TOV': per
        }
...
for f in sorted(archivos):
    try:
        ...
        df_temp = df_temp.sort_values(by='%_Wins', ascending=False)
...
print(f"\n🚀 ¡LISTO! Todos los archivos enriquecidos generados en {path_salida_seasons}")
'''
# -*- coding: utf-8 -*-
import pandas as pd
import os
...
print(f"\n🚀 ¡LISTO! Todos los archivos enriquecidos por fase generados en {path_salida_seasons}")