...
def crear_modelo_relacional_jugadores():
    ...
    try:
        ...
        # 1. DIMENSIÓN: Solo ID Jugador y Nombre (Entidad única sin duplicados)
        ...
        # Ordenamos alfabéticamente por nombre para que quede estético
        ...
        # Definimos el orden estricto de las columnas que me has pedido
        cols_fact = [
        ...
        # Filtramos asegurándonos de que solo vayan las columnas que existen en tu lista
        # (Esto evita errores si hay alguna pequeña diferencia en mayúsculas/minúsculas)
        cols_existentes = [c for c in cols_fact if c in df.columns]
        df_fact = df[cols_existentes].copy()
        
        # Ordenamos la Fact para que sea súper cómoda de analizar visualmente
