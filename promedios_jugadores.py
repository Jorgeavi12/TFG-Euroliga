...
def generar_promedios():
    ...
    if not os.path.exists(ruta_completa):
        print(f"No encuentro el archivo: {archivo_csv}")
        return

    df = pd.read_csv(ruta_completa, sep=';', encoding='utf-8-sig')

    # TUS COLUMNAS ORIGINALES (Con OREB% y DREB%, sin PER)
    cols_a_sumar = [
        ...
        'Mas_Menos', 'Game_Score', 'Minutos_Dec', 'OREB%', 'DREB%'
    ]
    ...
    # 4. ORDEN FINAL DE COLUMNAS (ID Temporada colocada rigurosamente en la PRIMERA posición)
    orden_final = [
    ...
    print(f"✅ Promedios estructurados por Temporada y Fase completados para {ANIO_OBJETIVO}")
