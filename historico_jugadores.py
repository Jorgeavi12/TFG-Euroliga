...
def generar_historico_jugadores_final():
    ...
    if not os.path.exists(ruta_unificado_origen):
        print("❌ Error: No se encuentra el archivo EuroleagueAllSeasonsJugadores.csv.")
        return
    ...
    try:
        ...
        print("⚙️ Procesando y acumulando trayectoria de IDs de equipos y volúmenes absolutos...")
        acumulado = {}
        ...
        for _, fila in df.iterrows():
            ...
            # CORRECCIÓN AQUÍ: Capturamos directamente 'ID_Equipo' en lugar de 'Equipo'
        ...
        print(f"❌ Error al procesar el archivo maestro: {e}")
        return

    lista_final = []
    for id_jugador, datos in acumulado.items():
        # Juntamos los códigos cortos (Ej: "BAR, OLY, RMD")
    ...
    print(f"📊 Columna 'ID_Equipo' corregida. Contiene únicamente códigos limpios (Ej: BAR, OLY, RMD).")
